...
def plot_rg(filename1,figout):
    ...
    ax.plot(position_z,densitys[:,0],color=cm.colors[0],label='0-2 us',alpha=0.8)
    ax.plot(position_z,densitys[:,1],color=cm.colors[2],label='2-4 us',alpha=0.8)
    ax.plot(position_z,densitys[:,2],color=cm.colors[4],label='4-6 us',alpha=0.8)
...
plot_rg('density-collect-6000ns-interval2000ns.xvg','density.png')
